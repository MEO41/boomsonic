...
json.dump(lines, open(os.path.join(ROOT, "data", "phase4", f"comp_lines_{TAG}{SUF}.json"), "w"), default=float)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(sls + dash).to_csv(os.path.join(ROOT, "data", f"phase4_running_line_{TAG}{SUF}.csv"), index=False)
    json.dump(dict(tag=TAG, R_design=R_d, design_check=dp, lines=[{k: L[k] for k in ("N", "W_choke", "W_peak", "PR_peak", "W_howell", "PR_howell", "row_howell", "peak_is_map_end")} for L in lines],
                   turbine_cover=cover), open(os.path.join(ROOT, "data", f"phase4_operability_{TAG}{SUF}.json"), "w"), indent=1, default=float)
