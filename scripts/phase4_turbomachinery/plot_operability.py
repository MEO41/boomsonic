...
L = json.load(open(os.path.join(ROOT, "data", "phase4", f"comp_lines_{TAG}_nw1.json")))
Wd, PRd = 1.0597, 5.03
rl = pd.read_csv(os.path.join(ROOT, "data", f"phase4_running_line_{TAG}_nw1.csv"))
nz = pd.concat([pd.read_csv(os.path.join(ROOT, "data", f)) for f in (f"phase4_nozzle_sweep_{TAG}.csv", f"phase4_nozzle_sweep_{TAG}_1.3-1.6-2.0.csv")])
bl = pd.concat([pd.read_csv(os.path.join(ROOT, "data", f)) for f in (f"phase4_bleed_sweep_{TAG}.csv", f"phase4_bleed_sweep_{TAG}_0.2-0.3.csv")])
bl = bl[~((bl.case.isin(["SLS_bleed20", "SLS_bleed30"])) & (bl.index < 0))]
ax5 = pd.read_csv(os.path.join(ROOT, "data", f"phase4_running_line_axi5_{TAG}.csv"))
...
fig.tight_layout(); fig.savefig(os.path.join(ROOT, "plots", "phase4_operability.png"), dpi=130)
print("saved")
