...
OUT = os.path.join(AXROOT, "data", "phase4")
...
if not os.path.exists(tmap_file):
    r = subprocess.run([NP1, os.path.join(ROOT, "scripts", "phase4_turbomachinery", "turbine_map.py"), os.path.join(ROOT, "data", "phase3", f"{TAG}_tt_out.json"), tmap_file], capture_output=True, text=True)
...
if __name__ == "__main__":
    ...
    df.to_csv(os.path.join(AXROOT, "data", f"phase4_running_line_{TAG}_nw{NEGW:g}{SUF}.csv"), index=False)
    json.dump(dict(tag=TAG, neg_width=NEGW, R_design=R_d, design=d, lines=[{k: L[k] for k in ("N", "W_choke", "W_peak", "PR_peak", "W_howell", "PR_howell", "row_howell")} for L in lines],
              ...
              open(os.path.join(AXROOT, "data", f"phase4_operability_{TAG}_nw{NEGW:g}{SUF}.json"), "w"), indent=1, default=float)
