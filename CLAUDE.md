...
**Phase 7 (afterburner) was run on the user's instruction, 2026-09-15 (design_log D7.0-D7.6; `docs/phase7_afterburner.md`).** Scope chosen by the user: extend the envelope past Mach 1, and decide the variable nozzle by trading three concepts on numbers. Scripts `scripts/phase7_afterburner/`, data `data/phase7/`, figures `plots/phase7_*.png`. **It is not a freeze, it answers none of the freeze's open decisions, and it does not touch Phase 6 CAD or the `axial/` tree.** Results: +79 % net thrust at the dash (500 -> 896 N at Tt7 1900 K) for +4.08 kg and +458 mm of engine length (CAD figure; the closure charged a conservative 484 mm); the envelope is capped at **M 1.33 by the turbine exit annulus**, not by thrust (R7.1, the centrifugal counterpart of Phase 6A's F6A.5 -- pre-existing, found not created, not fixed); a translating-plug nozzle wins the nozzle trade on actuation load (123 N vs the iris's 1198 N); TOGW closes at **24.78 kg only for brief afterburner use** and **fails at 26.02 kg** if the afterburner runs through the whole acceleration. **Phase 7 makes freeze risks 4.1/4.2 and 4.4 worse** (the whole benefit rests on the variable nozzle including its untested transients; screech was not checked against the impeller modes). New open risks **R7.1-R7.8** must not be presented as resolved.

**Phase 7 CAD (user instruction, 2026-09-15; design_log D7.7, F7.13-F7.15).** `scripts/phase7_afterburner/ab_cad.py` (`.venv-cad`) builds `cad/engine_with_afterburner_assembly.step` **on top of** the Phase 6 engine: it imports all of `cad/engine/*.step` unchanged and drops only `nozzle_outer_cone` and `tail_cone`, which the afterburner replaces. 25 Phase 6 parts carried over, 17 new in `cad/afterburner/`, 42/42 solids valid. Figures via `ab_render.py`. Findings: the afterburner flow path fits inside the 186.6 mm envelope with a 10.2 mm annulus, but **the nozzle actuator fits nowhere** (+25.0 mm over the envelope; the engine-to-skin gap is 8.7 mm) -- Phase 6A's F6A.2 repeated, new risk **R7.8**; and CAD mass is **+0.298 kg** over the bottom-up model, which puts TOGW at **25.10 kg, 0.10 kg over the ceiling** (the duct-Mach-0.30 build would be 23.80 kg). Radial parts are drawn on **+y** so they appear in the meridional z = 0 section.

**The afterburner lives in `cycle_model.py` as `afterburner=False` by default.** Options `afterburner`, `ab_mode` ('FAR' / 'T7') and `ab_mode_design` (the core is always sized dry); `read()` takes `eta_ab`; `set_design()` takes `ab_dPqP`, `ab_FAR`, `T7_K`. With the option off the model is bit-for-bit the Phase 3/4 model -- verified by `scripts/phase7_afterburner/verify_ab_cycle.py`, which must keep passing.
...
Phase 7: `.venv\Scripts\python scripts\phase7_afterburnererify_ab_cycle.py` (Rayleigh relations vs tables, the afterburner-off regression against the frozen dash point, the null-afterburner identity, fuel bookkeeping, Cantera heat release, closed-form A8 and Fg) and `ab_envelope.py --regress` (the real-map dry deck against `data/phase3r_mission_<tag>_deck.csv`, which must stay at 0.000 %).
...
* **pyCycle tabular thermo (Phase 7):** the `AIR_JETA_TAB_SPEC` FAR axis stops at **0.050** (stoichiometric Jet-A is 0.068) and **extrapolates silently**; Tt7 ~ 2000 K is the ceiling of the data, not physics (F7.1). Its `Combustor` applies `dPqP` as a fixed fraction and does **not** model the Rayleigh loss of heat addition -- 2.7 % of Pt in an afterburner at MN 0.20 (F7.2). Its tabular fuel carries **zero injection enthalpy** in the table datum, so an internal energy balance is vacuous; cross-check against Cantera instead.
* **Cantera `nDodecane_Reitz.yaml`:** the default phase `nDodecane_RK` is Redlich-Kwong and **every reactor and flame object rejects it** -- use `nDodecane_IG` (F7.3). It carries **no gas-phase transport data for c12h26**, so no laminar flame speed can be solved with it (F7.11). A stirred-reactor blowout search must use **continuation, not bisection** (a PSR is bistable), and each reservoir/reactor needs its **own `Solution`** or the "fresh" feed tracks the reactor (F7.10).
